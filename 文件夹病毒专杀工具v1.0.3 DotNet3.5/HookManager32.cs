    ...
    class HookManager32 : HookManager
    {
        ...
        public bool EnableHook()
        {
            try
            {
                ...
                SetCallBack(func);
                Logger.Info("正在拉起Hook安装代码");
                int result = Install();
                if (result == 0)
                {
                    Logger.Info("Hook安装失败");
                    return false;
                }
                Statue = true;
                return true;
        ...
        public static int ShowMessage(int a, IntPtr ptr)
        {
            ...
            {
                case 0:
                    Logger.Info("HookDll32", "拦截CreateFile请求:" + s);
                    break;
                case 1:
                    Logger.Info("HookDll32", s);
                    break;
                case 2:
                    Logger.Warn("HookDll32", s);
                    break;
                case 3:
                    Logger.Info("HookDll32", "Debug >" + s);
                    break;
            }
            return 0;
        }*/
        const string DllName = "InjectorDll32.dll";
        const string LogName = "InjectorDll32";
        public bool Statue { get; private set; } = false;
        ...
        */
        ...
        void EnumAndInject()
        {
            try
            {
                Logger.Info(LogName, "正在安装CallBack函数");
                SetCallBack(func);
            }
            catch
            {
